...
from app.packages.modulos.store import TiendaModule
...
def override_db():
    ...
    with DBSession(engine) as db:
        ...
        db.execute(
            text(
                "DELETE FROM usuarios_sitio "
                "WHERE id = 1 OR correo = 'store@test.com'"
            )
        )
...
async def test_store_flujo_completo():
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url=BASE_URL) as ac:
        admin_headers = await get_admin_headers(
            ac,
            f"store-admin-{uid}@test.com",
        )

        resp_404_site = await ac.get(
            "/api/v1/sitios/999999/tienda/productos"
        )
        assert resp_404_site.status_code == 200
        ...
        resp_cat = await ac.post(
            f"/api/v1/sitios/{site_db_id}/tienda/categorias",
            json=cat_data,
        ...
        resp_cat_get = await ac.get(
            f"/api/v1/sitios/{site_db_id}/tienda/categorias"
        )
        assert resp_cat_get.status_code == 200

        resp_cat_one = await ac.get(
            f"/api/v1/sitios/{site_db_id}/tienda/categorias/{cat_id}"
        )
        assert resp_cat_one.status_code == 200

        resp_cat_upd = await ac.put(
            f"/api/v1/sitios/{site_db_id}/tienda/categorias/{cat_id}",
            json={"nombre": "Laptops Gamer"},
            headers=admin_headers,
        )
        assert resp_cat_upd.status_code == 200

        await ac.get(
            f"/api/v1/sitios/{site_db_id}/tienda/categorias/999999"
        )
        ...
        resp_prod_1 = await ac.post(
            f"/api/v1/sitios/{site_db_id}/tienda/productos",
            json=prod_data_1,
            headers=admin_headers,
        )
        ...
        await ac.get(
            f"/api/v1/sitios/{site_db_id}/tienda/productos?featured=true"
        )

        resp_prod_one = await ac.get(
            f"/api/v1/sitios/{site_db_id}/tienda/productos/{prod_id_1}"
        )
        assert resp_prod_one.status_code == 200

        resp_prod_upd = await ac.put(
            f"/api/v1/sitios/{site_db_id}/tienda/productos/{prod_id_1}",
            json={"precio": 1200.0},
            headers=admin_headers,
        )
        ...
        await ac.get(
            f"/api/v1/sitios/{site_db_id}/tienda/productos/999999"
        )
        ...
        resp_cart_empty = await ac.get(
            f"/api/v1/sitios/{site_db_id}/tienda/carrito"
        )
        assert resp_cart_empty.json()["id"] == 0

        resp_cart_bad = await ac.post(
            f"/api/v1/sitios/{site_db_id}/tienda/carrito/items",
            json={"producto_id": prod_id_1, "cantidad": 1},
        )
        assert resp_cart_bad.status_code == 401

        item_data = {
            "producto_id": prod_id_1,
            "cantidad": 2,
            "usuario_id": 1,
        }

        resp_item = await ac.post(
            f"/api/v1/sitios/{site_db_id}/tienda/carrito/items",
            json=item_data,
        )
        ...
        resp_item_upd = await ac.put(
            f"/api/v1/sitios/{site_db_id}/tienda/carrito/items/{item_id}",
            params={"cantidad": 4},
        )
        assert resp_item_upd.status_code == 200

        await ac.put(
            f"/api/v1/sitios/{site_db_id}/tienda/carrito/items/999999",
            params={"cantidad": 1},
        )
        ...
        resp_checkout = await ac.post(
            f"/api/v1/sitios/{site_db_id}/tienda/checkout",
            json=checkout_data,
        )
        assert resp_checkout.status_code == 200
        pedido_id = resp_checkout.json()["pedido"]["id"]

        resp_checkout_empty = await ac.post(
            f"/api/v1/sitios/{site_db_id}/tienda/checkout",
            json=checkout_data,
        )
        assert resp_checkout_empty.status_code == 400

        resp_pedidos = await ac.get(
            f"/api/v1/sitios/{site_db_id}/tienda/pedidos",
            headers=admin_headers,
        )
        assert resp_pedidos.status_code == 200

        resp_pedido_one = await ac.get(
            f"/api/v1/sitios/{site_db_id}/tienda/pedidos/{pedido_id}",
            headers=admin_headers,
        )
        assert resp_pedido_one.status_code == 200

        resp_pedido_upd = await ac.put(
            f"/api/v1/sitios/{site_db_id}/tienda/pedidos/{pedido_id}/estado",
            json={"estado": "procesando"},
            headers=admin_headers,
        )
        assert resp_pedido_upd.status_code == 200

        resp_pedido_bad_state = await ac.put(
            f"/api/v1/sitios/{site_db_id}/tienda/pedidos/{pedido_id}/estado",
        ...
        await ac.get(
            f"/api/v1/sitios/{site_db_id}/tienda/pedidos/999999",
            headers=admin_headers,
        )

        await ac.put(
            f"/api/v1/sitios/{site_db_id}/tienda/pedidos/999999/estado",
        ...
        await ac.put(
            f"/api/v1/sitios/{site_db_id}/tienda/productos/{prod_id_2}",
            json={"stock": 1},
            headers=admin_headers,
        )

        resp_checkout_fail = await ac.post(
            f"/api/v1/sitios/{site_db_id}/tienda/checkout",
            json=checkout_data,
        )
...
async def test_store_excepciones_y_modulo():
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url=BASE_URL) as ac:
        ...
        with patch(
            "app.packages.modulos.store.services.StoreService.agregar_al_carrito",
        ...
        with patch(
            "app.packages.modulos.store.services.StoreService.crear_pedido",
            side_effect=Exception("DB Error"),
        ):
            resp = await ac.post(
                f"/api/v1/sitios/{site_db_id}/tienda/checkout",
        ...
        with patch(
            "app.packages.modulos.store.routes.StoreService",
            side_effect=Exception("DB Error"),
        ):
            resp = await ac.get(
                f"/api/v1/sitios/{site_db_id}/tienda/carrito?usuario_id=1"
...
async def test_store_routes_sitio_no_encontrado():
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url=BASE_URL) as ac:
        admin_headers = await get_admin_headers(
            ac,
            f"store-404-{uuid.uuid4().hex[:6]}@test.com",
        )
        ...
        resp = await ac.get(
            f"/api/v1/sitios/{site_bad}/tienda/pedidos",
            headers=admin_headers,
        )
        assert resp.status_code == 404

        resp = await ac.get(
            f"/api/v1/sitios/{site_bad}/tienda/pedidos/1",
            headers=admin_headers,
        )
        assert resp.status_code == 404
        ...
        resp = await ac.get(
        f"/api/v1/sitios/{site_bad}/tienda/carrito?usuario_id=1"
)
        assert resp.status_code == 404
...
async def test_store_routes_upload_image_validaciones_y_ok():
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url=BASE_URL) as ac:
        headers = await get_admin_headers(
            ac,
            f"store-upload-{uuid.uuid4().hex[:6]}@test.com",
        )
